...
/// Bounds how many connection-handler threads can be alive at once --
/// unbounded thread-per-connection means a client that just holds
/// connections open (never sending a full request) can exhaust threads
/// or file descriptors. Not a full I/O timeout (a slow client still
/// occupies one of these slots for as long as it stays connected), just
/// a cap on the worst case.
...
/// normal operation.
pub fn acceptLoop(io: Io, gpa: std.mem.Allocator, server: *Io.net.Server, limiter: *ConnectionLimiter) void {
    while (true) {
        const conn = server.accept(io) catch return;

        if (limiter.active.fetchAdd(1, .monotonic) >= limiter.max) {
            _ = limiter.active.fetchSub(1, .monotonic);
            conn.close(io);
            continue;
        }

        const thread = std.Thread.spawn(.{}, handleConnection, .{ io, gpa, conn, limiter }) catch {
            _ = limiter.active.fetchSub(1, .monotonic);
            conn.close(io);
            continue;
        };
...
pub fn serve(io: Io, gpa: std.mem.Allocator, address: *const Io.net.IpAddress) !void {
    ...
    var limiter = ConnectionLimiter{ .max = default_max_connections };
    acceptLoop(io, gpa, &server, &limiter);
}

fn handleConnection(io: Io, gpa: std.mem.Allocator, conn: Io.net.Stream, limiter: *ConnectionLimiter) void {
    defer _ = limiter.active.fetchSub(1, .monotonic);
    defer conn.close(io);
...
/// `max_connections` is explicit (not defaulted) so the connection-cap
/// test can pass a tiny value while every other test passes a generous
/// one that its handful of connections will never approach.
fn startTestServer(io: Io, request_gpa: std.mem.Allocator, address: *const Io.net.IpAddress, max_connections: u32) !TestServer {
    ...
    // Never freed, same reasoning as capture_loop.zig's g_slot.jpeg:
    // stop() joins acceptLoop itself but not the detached handleConnection
    // threads it spawned, and a still-finishing one (e.g. a client that
    // closes right as the test ends) touches this pointer in its own
    // deferred cleanup -- freeing it here raced that into a real,
    // ReleaseSmall-reproducible use-after-free.
    const limiter = try std.heap.page_allocator.create(ConnectionLimiter);
    limiter.* = .{ .max = max_connections };
    const accept_thread = try std.Thread.spawn(.{}, acceptLoop, .{ io, request_gpa, server, limiter });
...
test "GET / returns 200 with a webcam2ip body" {
    ...
    const test_server = try startTestServer(io, std.testing.allocator, &address, 64);
...
test "GET /snapshot.jpg returns the latest frame with an image/jpeg content-type" {
    ...
    const test_server = try startTestServer(io, gpa, &address, 64);
    defer test_server.stop();
...
test "acceptLoop closes a connection outright once the concurrent cap is reached" {
    const io = std.testing.io;
    const address = try Io.net.IpAddress.parseLiteral("127.0.0.1:17179");
    const test_server = try startTestServer(io, std.testing.allocator, &address, 2); // cap = 2
    defer test_server.stop();

    // Two connections that never finish their request line -- receiveHead()
    // blocks waiting for the rest, so these occupy both slots for as long
    // as this test holds them open.
    var held: [2]Io.net.Stream = undefined;
    for (&held) |*c| {
        c.* = try address.connect(io, .{ .mode = .stream });
        var write_buf: [16]u8 = undefined;
        var w = c.writer(io, &write_buf);
        try w.interface.writeAll("GET ");
        try w.interface.flush();
    }
    defer for (held) |c| c.close(io);
    try Io.sleep(io, .fromMilliseconds(100), .awake); // let acceptLoop accept + spawn both

    // A third connection is accepted at the TCP level but should be
    // closed immediately, without ever reaching a handler.
    var rejected = try address.connect(io, .{ .mode = .stream });
    defer rejected.close(io);

    var read_buf: [16]u8 = undefined;
    var rejected_reader = rejected.reader(io, &read_buf);
    try std.testing.expectError(error.EndOfStream, rejected_reader.interface.take(1));
}
