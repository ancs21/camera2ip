...
fn onFrame(rgba: [*]u8, width: i32, height: i32, bytes_per_row: i32) callconv(.c) void {
    ...
    // Grow before clearing -- on failure, readers still see the old frame instead of a corrupt empty one.
    g_slot.jpeg.ensureTotalCapacity(g_gpa, bytes.len) catch return;
    g_slot.jpeg.clearRetainingCapacity();
    g_slot.jpeg.appendSliceAssumeCapacity(bytes);
    g_slot.width = width;
...
test "onFrame leaves the previous frame intact if the replacement allocation fails" {
    const io = std.testing.io;
    resetForTesting(io);
    g_gpa = std.heap.page_allocator;
    ...
    var red = [_]u8{ 255, 0, 0, 255 } ** 4; // 2x2 solid red
    onFrame(&red, 2, 2, 8);
    const before = (try copyLatestFrame(io, std.testing.allocator)).?;
    defer std.testing.allocator.free(before.data);
    try std.testing.expect(before.data.len > 0);

    // Past the throttle window, then force the *next* frame's own
    // allocation (growing g_slot.jpeg's capacity) to fail.
    try Io.sleep(io, .fromMilliseconds(150), .awake);
    // Both indices at 0: block the growth attempt whether it goes
    // through resize/remap-in-place or a fresh alloc.
    var failing = std.testing.FailingAllocator.init(std.heap.page_allocator, .{ .fail_index = 0, .resize_fail_index = 0 });
    g_gpa = failing.allocator();

    // A large, non-flat pattern: JPEG compresses a solid color down to
    // near the format's fixed header overhead, which would fit in the
    // red frame's already-oversized (amortized growth) capacity and
    // never actually touch the allocator. Noise defeats that.
    var noisy: [32 * 32 * 4]u8 = undefined;
    for (&noisy, 0..) |*byte, i| byte.* = @truncate(i *% 2654435761);
    onFrame(&noisy, 32, 32, 32 * 4);

    // The failed allocation must not have wiped the previous good frame.
    const after = (try copyLatestFrame(io, std.testing.allocator)).?;
    defer std.testing.allocator.free(after.data);
